...
def test_append_instructions_steps_along_x():
    ...
    x, y, _ = append_instructions(0, (1, 0), 5.0, 0.0, 0.0, np.array([[0.0, 0.0]]), float)
...
def test_append_instructions_steps_along_y():
    ...
    x, y, _ = append_instructions(0, (0, 1), 5.0, 0.0, 0.0, np.array([[0.0, 0.0]]), float)
...
def test_append_instructions_int_dtype_rounds_sin():
    ...
    z_float = append_instructions(np.pi / 4, (1, 0), 1.0, 0.0, 0.0, np.array([[0.0, 0.0]]), float)[2]
    z_int   = append_instructions(np.pi / 4, (1, 0), 1.0, 0.0, 0.0, np.array([[0.0, 0.0]]), int)[2]
...
def test_reduce_instructions_single_f_steps_right():
    z = np.array([[0.0, 0.0]])
    pts = reduce_instructions(0.0, 0.0, z, ((1, 'A'), (2, 'B'), (3, 'C')), 1.0, 'F', np.pi / 2, (1, 0), int)
    assert pts.shape == (2, 2)
...
def test_reduce_instructions_ff_uses_b_width():
    ...
    z = np.array([[0.0, 0.0]])
    pts = reduce_instructions(0.0, 0.0, z, ((1, 'A'), (2, 'B'), (3, 'C')), 1.0, 'FF', np.pi / 2, (1, 0), int)
    assert pts.shape == (2, 2)
...
def test_reduce_instructions_turn():
    z = np.array([[0.0, 0.0]])
    pts = reduce_instructions(0.0, 0.0, z, ((1, 'A'), (2, 'B'), (3, 'C')), 1.0, 'F+F', np.pi / 2, (1, 0), int)
    assert pts.shape == (3, 2)
    assert np.allclose(pts[1], [1.0, 0.0])
    assert np.allclose(pts[2], [1.0, -1.0])
